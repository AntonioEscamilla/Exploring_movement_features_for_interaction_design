...
def right_click(event):
    ...
    for i in range(num_strokes):
        traj = trajectory_transform(all_strokes[i], use_x0_translate=True)
        draw_stroke(np.array(traj * 500 + 250, np.int32))
    ...
    print(f'fast cluster labels: {labels}')

    dist_matrix = squareform(pdist)
    #print(dist_matrix)
    clustering = DBSCAN(eps=0.05, min_samples=2, metric='precomputed').fit(dist_matrix)
    print(f'DBSCAN labels: {clustering.labels_}')
